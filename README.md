...
- **ミックス・マスタリング実践** — `mix_mastering_edit`(実践的な編集/ミキシング/
  ...
  セッションで使える」判断基準を扱う編
- **実践問題編** — `gear_patch_practice`(デスク配線パズル、全36単元)、
  `money_case_studies`(お金のケーススタディ、全18単元)、`mix_tips_practice`
...
`courses/*.js` を編集したら、pushする前に必ずNode.jsで構文エラーが無いか
確認する(ブラウザの`<script>`読み込みでは構文エラーがコンソールに出るだけで
気づきにくいため)。対象ファイルごとに以下を実行する:

```sh
node -e "global.window={}; require('./courses/対象ファイル.js'); console.log('OK')"
```

`OK`が出力されればその場でファイルは構文的に読み込み可能。エラーが出た場合は
push前に必ず修正すること。
