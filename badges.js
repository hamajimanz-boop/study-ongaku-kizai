...
window.getAllBadges = function getAllBadges(state) {
  ...
  Object.keys(window.COURSES)
    ...
    .forEach((cid) => {
      ...
      badges.push({
        id: `course_master_${cid}`, title: `${course.title} マスター`,
  ...
  (window.GENRES || []).forEach((genre) => {
    ...
    if (!realUnits) return;
    badges.push({
      id: `genre_master_${genre.id}`, title: `${genre.title} 制覇賞`,
